...
def depth_first_search(problem,node,frontier,explored):
    if problem.goal_test(node.state):
        print node.solution(),node.path_cost
    while frontier:
        node=frontier.pop(0)
        ...
        for child in node.expand(problem):
            if child.state not in explored and child.state not in frontier:
                ...
                depth_first_search(problem,child,frontier,explored)
    return None

def dfs(problem,node):
    frontier=list()
    explored=set()
    frontier.append(node)
    depth_first_search(problem,node,frontier,explored)

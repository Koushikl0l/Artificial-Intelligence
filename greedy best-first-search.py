...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 22 19:29:55 2018
...
def heuristic_BaseFrontier(problem,frontier):
       if len(frontier)<=1:
           obj=frontier[0]
           return obj
       else:
           for i in range(0,len(frontier)):
               for  j in range(1,len(frontier)):
                   if problem.heuristic[frontier[i].state]>problem.heuristic[frontier[j].state]:
...
def greedy_best_first_search(problem):
    ...
    explored = set() 
    while frontier:
        node = heuristic_BaseFrontier(problem,frontier)
...
result=greedy_best_first_search(gp)
